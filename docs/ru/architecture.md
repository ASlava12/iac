...
2. Маршрутизирует ресурсы агентам по `spec.hostSelector.name`.
3. Считает слои (layers) по `metadata.dependsOn` (Phase 7by —
   поэтапное применение, phased apply).
...
* **Слои (layers)** (Phase 7by) приходят из `metadata.dependsOn` —
  список идентификаторов ресурсов вида `<kind>/<environment>/<name>`,
  которые должны завершиться раньше. Layer-N+1 не может стартовать,
  пока каждое задание layer-N не достигнет финального состояния
  (succeeded — успех). Сбой (failure) в любом слое отменяет все
  последующие. **Внимание:** сейчас поле читается сервером из сырого
  JSON ресурса; при отправке через `iac` CLI struct `iac-core::Metadata`
  отбрасывает неизвестные ключи при YAML→JSON round-trip, поэтому
  `metadata.dependsOn` из манифеста теряется до сервера. Известный
  пробел в CLI; временный обходной путь — отправлять операцию прямо
  через API (`POST /v1/operations`) с сохранённым полем.
