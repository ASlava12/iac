...
async fn spawn(
    ...
    let config_issues = Arc::new(
        iac_controlplane::maintenance::collect_config_issues(
...
async fn returns_empty_list_when_config_is_clean() {
    let server = spawn(
        vec![abs_window("good", "2026-05-01T02:00:00Z", "2026-05-01T04:00:00Z")],
...
async fn admin_only() {
    ...
    let token = reqwest::Client::new()
        ...
        .json(&LoginRequest { username: "op".into(), password: "p".into() })
        .send()
        .await
        .unwrap()
        .json::<LoginResponse>()
